...
class ReplyRepositoryPostgres extends ReplyRepository {
  ...
  async getRepliesByThreadId(threadId) {
    const query = {
      text: `SELECT replies.*, users.username FROM replies LEFT JOIN users
      ON users.id = replies.owner LEFT JOIN comments ON comments.id = replies.comment
      WHERE comments.thread = $1 ORDER BY replies.date ASC`,
      values: [threadId],
    };
